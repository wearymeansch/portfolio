...
def make_insert(s,zn):#функция вызывается для вставки в корень s
    s=insert(s, zn)
    if s[4]=='r':#аналог случая 1 в Википедии
        s[4]='b'
    return s
def grandparent(s):
    if s[3]:
...
def insert(s,zn):#функция вызывается для вставки в произвольное место дерева s
    if s == None:#аналог случая 2 в Википедии
        s=[zn, None, None, None,"r"]#создание нового эл-та, который к чему-нибудь будет привязан
    elif s[0]>zn:
        s[1]=insert(s[1], zn)#вставка числа в левую ветку
        s[1][3]=s
        if s[4]=='r' and s[1][4]=='r':
            s[4]='b'
        elif s[2] and s[2][4]=='r':#перекраска красного дяди в черный цвет
            s[2][4]='b'
            s[4]='r'
    else:
        s[2]=insert(s[2], zn)#вставка числа в правую ветку
        s[2][3]=s
        if s[4]=='r' and s[2][4]=='r':
            s[4]='b'
        elif s[1] and s[1][4]=='r':#перекраска красного дяди в черный цвет
            s[1][4]='b'
            s[4]='r'
    return s
