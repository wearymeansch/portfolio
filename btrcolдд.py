...
def insert(s,zn):#функция вызывается для вставки в произвольное место дерева s
    ...
    if s == None:#аналог случая 2 в Википедии
        ...
        s[1],da=insert(s[1], zn)#вставка числа в левую ветку
        if s[3]=='r' and s[1][3]=='r':
            s[3]='b'
            dr=DB
        elif s[2] and s[2][3]=='r' and da==DB:#перекраска красного дяди в черный цвет
            s[2][3]='b'
            s[3]='r'
            dr=DN
    else:
        s[2],da=insert(s[2], zn)#вставка числа в правую ветку
        print('da=', da)
        if s[3]=='r' and s[2][3]=='r':
            s[3]='b'
            dr=DB
        elif s[1] and s[1][3]=='r' and da==DB:#перекраска красного дяди в черный цвет
            s[1][3]='b'
            s[3]='r'
            dr=DN
    return s, dr
